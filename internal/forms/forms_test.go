...
func TestForm_Has(t *testing.T) {
	...
	has = form.Has("a")
	if !has {
		t.Error("form shows does not have fields when it should")
	}
}
...
func TestForm_MinLength(t *testing.T) {
	...
	if !form.Valid() {
		t.Error("form shows min length of 1 is not met when it does")
	}
...
func TestForm_IsEmail(t *testing.T) {
	...
	form.IsEmail("email")
	if !form.Valid() {
		t.Error("form shows an invalid email when we should not have")
	}
	...
	form.IsEmail("email")
	if form.Valid() {
		t.Error("form shows an valid email when we should be invalid")
	}
}
